...
if game.PlaceId ~= 126884695634066 then return end -- only run in this game (Grow a Garden)
...
uis.InputBegan:Connect(function(input, gpe)
    ...
    or uis:IsKeyDown(Enum.KeyCode.LeftMeta) or uis:IsKeyDown(Enum.KeyCode.RightMeta)) then
        if input.KeyCode == Enum.KeyCode.One then
            game.ReplicatedStorage.GameEvents.PetsService:FireServer("SwapPetLoadout", mapping[1])
        elseif input.KeyCode == Enum.KeyCode.Two then
            game.ReplicatedStorage.GameEvents.PetsService:FireServer("SwapPetLoadout", mapping[2])
        elseif input.KeyCode == Enum.KeyCode.Three then
            game.ReplicatedStorage.GameEvents.PetsService:FireServer("SwapPetLoadout", mapping[3])
        end
    end
end)